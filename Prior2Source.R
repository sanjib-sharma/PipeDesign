#BIC
gevbic<-function(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma,a_xi){
  bic<--2*newloglik(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma,a_xi)+(log(length(data))*length(c(mu0,sigma_0,xi_0,a_mu,a_sigma,a_xi)))
  return(bic)
}

#AIC
gevaic<-function(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma,a_xi){
  aic<-2*length(c(mu0,sigma_0,xi_0,a_mu,a_sigma,a_xi))-2*newloglik(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma,a_xi)
  return(aic)
}
...
newloglik<-function(dataset,tempset1,mu_0,sigma_0,xi_0,a_mu,a_sigma,a_xi)
{
  MU<-mu_0*(1+a_mu*tempset1)
  SIGMA<-exp(sigma_0*(1+a_sigma*tempset1))
  XI<-xi_0*(1+a_xi*tempset1)
...
MetNonstatGEVspsl<-function(dataset,tempset1,n,start,errvect,uvect,sdvect){
  # Start the setup
  cand<-matrix(start,1,6,byrow=TRUE); colnames(cand)<-c("mu0","sigma0","xi0","amu","asigma","axi")
  finmat<-matrix(start,1,6,byrow=TRUE); colnames(finmat)<-c("mu0","sigma0","xi0","amu","asigma","axi")
  aprobmat<-matrix(NA,1,6,byrow=TRUE); colnames(aprobmat)<-c("mu0","sigma0","xi0","amu","asigma","axi")
  errsdmat<-matrix(c(errvect,uvect,sdvect),3,6,byrow=TRUE); colnames(errsdmat)<-c("mu0","sigma0","xi0","amu","asigma","axi")
...
MULTnsgevplots<-function(finlist,rm.burn=FALSE,burn=1000){
  ...
  finamu<-finlist$finmat[,4];  finasigma<-finlist$finmat[,5];  finaxi<-finlist$finmat[,6]
  ...
  if(rm.burn==FALSE){
    ...
    abline(v=burn,col="red")
    plot(ts(finasigma),ylab="value",xlab="Iteration",main="Markov Chain Results-Asigma")
    abline(v=burn,col="red")
    plot(ts(finaxi),ylab="value",xlab="Iteration",main="Markov Chain Results-Axi")
    abline(v=burn,col="red")
    ...
    asigma.burn<-finasigma[burn:length(finasigma)]
    axi.burn<-finaxi[burn:length(finaxi)]
    ...
    plot(ts(asigma.burn),ylab="value",xlab="Iteration",main="Markov Chain Results-Asigma")
    plot(ts(axi.burn),ylab="value",xlab="Iteration",main="Markov Chain Results-Axi")
...
MULTncrej<-function(finlist,burn=1000){
  ...
  aprobasigma.burn<-finlist$aprobmat[-(1:burn),5]
  aprobaxi.burn<-finlist$aprobmat[-(1:burn),6]
  
  
  rejprob<-data.frame(parameter=c( "mu0","sigma0","xi0","amu","asigma","axi"),
                      mean_acceptance_rate=c(
                        ...
                        round(mean(aprobamu.burn,na.rm=TRUE),4),round(mean(aprobasigma.burn,na.rm=TRUE),4),round(mean(aprobaxi.burn,na.rm=TRUE),4)     
                      ))
  return(rejprob)
}
...
DensePlot<-function(finlist,burn=1000){
  ...
  amu.burn<-finamu[burn:length(finamu)];asigma.burn<-finasigma[burn:length(finasigma)];axi.burn<-finaxi[burn:length(finaxi)];
  ...
  asigma_hpd<-hpd(asigma.burn,p=0.05);asigma_hpd_b<-hpd(asigma.burn,p=0.1)
  axi_hpd<-hpd(axi.burn,p=0.05);axi_hpd_b<-hpd(axi.burn,p=0.1)
  ...
  asigmadat<-data.frame(asigma.x=density(asigma.burn)$x,asigma.y=density(asigma.burn)$y)
  asigmamode<-mean(asigma.burn)
  
  axidat<-data.frame(axi.x=density(axi.burn)$x,axi.y=density(axi.burn)$y)
  aximode<-mean(axi.burn)
  ...
  plot(density(asigma.burn),main=expression(paste("Density for sampled asigma")))
  abline(v = asigmamode,col="red",lwd=3)
  abline(v = asigma_hpd[1,1],col="gray",lwd=3,lty="dotted")
  abline(v = asigma_hpd[1,2],col="gray",lwd=3,lty="dotted")
  abline(v = asigma_hpd_b[1,1],col="green",lwd=3,lty="dotted")
  abline(v = asigma_hpd_b[1,2],col="green",lwd=3,lty="dotted")
  
  plot(density(axi.burn),main=expression(paste("Density for sampled axi")))
  abline(v = aximode,col="red",lwd=3)
  abline(v = axi_hpd[1,1],col="gray",lwd=3,lty="dotted")
  abline(v = axi_hpd[1,2],col="gray",lwd=3,lty="dotted")
  abline(v = axi_hpd_b[1,1],col="green",lwd=3,lty="dotted")
  abline(v = axi_hpd_b[1,2],col="green",lwd=3,lty="dotted")
...
posteriormodeGEV<-function(finlist,burn=1000){
  ...
  modeframe=data.frame(pred=c("mu0","sigma0","xi0","amu","asigma","axi"),
                       mode=c(mumode,sigmamode,ximode,amumode,asigmamode,aximode
                       ))
  return(modeframe)
}
...
BM_MCMC<-function(finlist,int=1000,burn=0,end=nrow(finlist$finmat),bline=0){
  ...
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(bmat[,1]~bmat[,7],typ="l",main="BM for mu0");abline(v=bline,col="red")  
  plot(bmat[,2]~bmat[,7],typ="l",main="BM for sigma0");abline(v=bline,col="red")  
  plot(bmat[,3]~bmat[,7],typ="l",main="BM for xi0");abline(v=bline,col="red")  
  plot(bmat[,4]~bmat[,7],typ="l",main="BM for amu");abline(v=bline,col="red")  
  plot(bmat[,5]~bmat[,7],typ="l",main="BM for asigma");abline(v=bline,col="red")  
  plot(bmat[,6]~bmat[,7],typ="l",main="BM for axi");abline(v=bline,col="red") 
...
plotonlyBM<-function(finlist,burn=1){
  bmat<-finlist$se
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(bmat[,1]~bmat[,7],typ="l",main="BM for mu0")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,2]~bmat[,7],typ="l",main="BM for sigma0")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,3]~bmat[,7],typ="l",main="BM for xi0")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,4]~bmat[,7],typ="l",main="BM for amu")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,5]~bmat[,7],typ="l",main="BM for asigma")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,6]~bmat[,7],typ="l",main="BM for axi")
  abline(v = burn,col="red",lwd=3) 
}
...
Mean_MCMC<-function(finlist,int=1000,burn=0,end=nrow(finlist$finmat),bline=0){
  ...
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(meanmat[,1]~meanmat[,7],typ="l",main="Sample Mean for mu0");abline(v=bline,col="red")  
  plot((meanmat[,2])~meanmat[,7],typ="l",main="Sample Mean for sigma0");abline(v=bline,col="red")  
  plot(meanmat[,3]~meanmat[,7],typ="l",main="Sample Mean for xi0");abline(v=bline,col="red")  
  plot(meanmat[,4]~meanmat[,7],typ="l",main="Sample Mean for amu");abline(v=bline,col="red")  
  plot(meanmat[,5]~meanmat[,7],typ="l",main="Sample Mean for asigma");abline(v=bline,col="red")  
  plot(meanmat[,6]~meanmat[,7],typ="l",main="Sample Mean for axi");abline(v=bline,col="red")  
...
plotonlyMean<-function(meanmat,burn=1){
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(meanmat[,1]~meanmat[,7],typ="l",main="Sample Mean for mu0")
  abline(v = burn,col="red",lwd=3) 
  plot((meanmat[,2])~meanmat[,7],typ="l",main="Sample Mean for sigma0")
  abline(v = burn,col="red",lwd=3) 
  plot(meanmat[,3]~meanmat[,7],typ="l",main="Sample Mean for xi0")
  abline(v = burn,col="red",lwd=3) 
  plot(meanmat[,4]~meanmat[,7],typ="l",main="Sample Mean for amu")
  abline(v = burn,col="red",lwd=3) 
  plot(meanmat[,5]~meanmat[,7],typ="l",main="Sample Mean for asigma")
  abline(v = burn,col="red",lwd=3) 
  plot(meanmat[,6]~meanmat[,7],typ="l",main="Sample Mean for axi")
  abline(v = burn,col="red",lwd=3) 
}
...
CredIntervalsGEV<-function(finlist,burn=1000){
  ...
  ####figures
  credints<-data.frame(parameter=c( "mu0","sigma0","xi0","amu","asigma","axi"),
                       posterior_mode=c(mumode,sigmamode,ximode,amumode,asigmamode,aximode),
...
BoxPlotGEV<-function(finlist,burn=1000){
  ...
  boxplot(mu0.burn,sigma0.burn,xi0.burn,amu.burn,asigma.burn,axi.burn,names=c("mu0","sigma0","xi0","amu","asigma","axi"),
...
optimizemat<-function(int,iter,dataset,tempset1,
  ...
  testparmat<-matrix(ncol=6,nrow=iter,NA)
  testrejmat<-matrix(ncol=6,nrow=iter,NA)
  start_sim<-start_sim+c(rgamma(1,0.25,1),rgamma(1,0.25,1),rgamma(1,0.25,1),rnorm(3,0,0.5))
