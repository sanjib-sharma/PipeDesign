#BIC
gevbic<-function(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma){
  bic<--2*newloglik(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma)+
    (log(length(data))*length(c(mu0,sigma_0,xi_0,a_mu,a_sigma)))
  return(bic)
}

#AIC
gevaic<-function(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma){
  aic<-(2*length(c(mu0,sigma_0,xi_0,a_mu,a_sigma)))-2*newloglik(data,temps,mu0,sigma_0,xi_0,a_mu,a_sigma)
      return(aic)
}
...
newloglik<-function(dataset,tempset1,mu_0,sigma_0,xi_0,a_mu,a_sigma)
{
...
MetNonstatGEVspsl<-function(dataset,tempset1,n,start,errvect,uvect,sdvect){
  # Start the setup
  cand<-matrix(start,1,length(start),byrow=TRUE); colnames(cand)<-c("mu0","sigma0","xi0","amu","asigma")
  finmat<-matrix(start,1,length(start),byrow=TRUE); colnames(finmat)<-c("mu0","sigma0","xi0","amu","asigma")
  aprobmat<-matrix(NA,1,length(start),byrow=TRUE); colnames(aprobmat)<-c("mu0","sigma0","xi0","amu","asigma")
  errsdmat<-matrix(c(errvect,uvect,sdvect),3,length(start),byrow=TRUE); colnames(errsdmat)<-c("mu0","sigma0","xi0","amu","asigma")
...
ResumeMetNonstatGEVspsl<-function(data,n){
  # Start the setup
  cand<-data$cand;finmat<-data$finmat;aprobmat<-data$aprobmat;
  errsdmat<-data$errsdmat;dataset<-data$dataset;tempset1<-data$tempset1
  # Metropolis Hastings Algorithm (Random Walk)
  for(i in (nrow(finmat)+1):n){
    cand<-rbind(cand,rep(NA,ncol(cand)));
    finmat<-rbind(finmat,rep(NA,ncol(finmat)));
...
MULTnsgevplots<-function(finlist,rm.burn=FALSE,burn=1000){
  ...
  finamu<-finlist$finmat[,4];  finasigma<-finlist$finmat[,5]
...
MULTncrej<-function(finlist,burn=1000){
  ...
  rejprob<-data.frame(parameter=c( "mu0","sigma0","xi0","amu","asigma"),
                      mean_acceptance_rate=c(
                        round(mean(aprobmu0.burn,na.rm=TRUE),4),round(mean(aprobsigma0.burn,na.rm=TRUE),4),
                        round(mean(aprobxi0.burn,na.rm=TRUE),4),round(mean(aprobamu.burn,na.rm=TRUE),4),
                        round(mean(aprobasigma.burn,na.rm=TRUE),4)     
                      ))
  return(rejprob)
}
...
DensePlot<-function(finlist,burn=1000){
  ...
  amu.burn<-finamu[burn:length(finamu)];asigma.burn<-finasigma[burn:length(finasigma)]
...
posteriormodeGEV<-function(finlist,burn=1000){
  ...
  modeframe=data.frame(pred=c("mu0","sigma0","xi0","amu","asigma"),
                       mode=c(mumode,sigmamode,ximode,amumode,asigmamode))
  return(modeframe)
}
...
BM_MCMC<-function(finlist,int=1000,burn=0,end=nrow(finlist$finmat),bline=0){
  ...
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(bmat[,1]~bmat[,ncol(bmat)],typ="l",main="BM for mu0");abline(v=bline,col="red")  
  plot(bmat[,2]~bmat[,ncol(bmat)],typ="l",main="BM for sigma0");abline(v=bline,col="red")  
  plot(bmat[,3]~bmat[,ncol(bmat)],typ="l",main="BM for xi0");abline(v=bline,col="red")  
  plot(bmat[,4]~bmat[,ncol(bmat)],typ="l",main="BM for amu");abline(v=bline,col="red")  
  plot(bmat[,5]~bmat[,ncol(bmat)],typ="l",main="BM for asigma");abline(v=bline,col="red")  
...
plotonlyBM<-function(finlist,burn=1){
  bmat<-finlist$se
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(bmat[,1]~bmat[,ncol(bmat)],typ="l",main="BM for mu0")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,2]~bmat[,ncol(bmat)],typ="l",main="BM for sigma0")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,3]~bmat[,ncol(bmat)],typ="l",main="BM for xi0")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,4]~bmat[,ncol(bmat)],typ="l",main="BM for amu")
  abline(v = burn,col="red",lwd=3) 
  plot(bmat[,5]~bmat[,ncol(bmat)],typ="l",main="BM for asigma")
  abline(v = burn,col="red",lwd=3) 
}
...
Mean_MCMC<-function(finlist,int=1000,burn=0,end=nrow(finlist$finmat),bline=0){
  ...
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(meanmat[,1]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for mu0");abline(v=bline,col="red")  
  plot(meanmat[,2]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for sigma0");abline(v=bline,col="red")  
  plot(meanmat[,3]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for xi0");abline(v=bline,col="red")  
  plot(meanmat[,4]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for amu");abline(v=bline,col="red")  
  plot(meanmat[,5]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for asigma");abline(v=bline,col="red")  
...
plotonlyMean<-function(meanmat,burn=1){
  par(mfrow=c(2,3),mar=c(2,2,2,2))
  plot(meanmat[,1]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for mu0")
  abline(v = burn,col="red",lwd=3) 
  plot((meanmat[,2])~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for sigma0")
  abline(v = burn,col="red",lwd=3) 
  plot(meanmat[,3]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for xi0")
  abline(v = burn,col="red",lwd=3) 
  plot(meanmat[,4]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for amu")
  abline(v = burn,col="red",lwd=3) 
  plot(meanmat[,5]~meanmat[,ncol(meanmat)],typ="l",main="Sample Mean for asigma")
  abline(v = burn,col="red",lwd=3) 
  }
...
CredIntervalsGEV<-function(finlist,burn=1000){
  ...
  ####figures
  credints<-data.frame(parameter=c( "mu0","sigma0","xi0","amu","asigma"),
                       posterior_mode=c(mumode,sigmamode,ximode,amumode,asigmamode),
...
BoxPlotGEV<-function(finlist,burn=1000){
  ...
  boxplot(mu0.burn,sigma0.burn,xi0.burn,amu.burn,asigma.burn,names=c("mu0","sigma0","xi0","amu","asigma"),
...
optimizemat<-function(int,iter,dataset,tempset1,
  ...
  testparmat<-matrix(ncol=length(start_sim),nrow=iter,NA)
  testrejmat<-matrix(ncol=length(start_sim),nrow=iter,NA)
  start_sim<-start_sim+c(rgamma(1,1,1),rgamma(1,1,1),rgamma(1,1,1),rnorm(2,0,2))
