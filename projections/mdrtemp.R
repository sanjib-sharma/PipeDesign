...
retint500<-benreturn(500,mu_chain,sigma_chain,xi_chain)
...
save(retint500,file=paste(main_path,"/MDRRESULT/stat_widenorm_rtnlevel500.RData",sep=""))
...
MULTncrej(run2,burn=burnin) #RejectionRate
cred.table<-CredIntervalsGEV(run2,burn=burnin) # Credible Intervals
save(run2,file=paste(main_path,"/MDRRESULT/Mu_nonstat_widenorm_run2_RCM.RData",sep=""))
 cred.table
 save(cred.table,file=paste(main_path,"/MDRRESULT/Mu_nonstat_widenorm_CI_RCM.RData",sep=""))
 mu0=run2$finmat[,1]
 amu=run2$finmat[,4]
 sigma=exp(run2$finmat[,2])
 xi=run2$finmat[,3]
