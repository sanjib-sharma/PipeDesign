...
save(retint500,file=paste(main_path,"/TSRESULT/stat_widenorm_rtnlevel500.RData",sep=""))
...
gg=2019
 ...
 sigma_chain <- sigma[(length(sigma)-40000+1):length(sigma)]
 xi_chain <- xi[(length(xi)-40000+1):length(xi)]
save(muproj_chain,xi_chain,sigma_chain,file=paste(main_path,'/TSRESULT/nonstat_param',gg,'.RData',sep=""))}
